...
double mathUtility::sawtoothFunction(double radians)
{
    // Function added by Calvin Lacher
    // Limits angles in ]0, 360[
    return 2*atan(tan(radians/2));
}
...
double mathUtility::calculateAngleOfDesiredTrajectory(const double nextLon, const double nextLat, 
    const double prevLon, const double prevLat, const double gpsLon, const double gpsLat)
{
    ...
    double M[2][3] = {
        {-sin(degreeToRadian(gpsLon)), cos(degreeToRadian(gpsLon )), 0},
        {-cos(degreeToRadian(gpsLon ))*sin(degreeToRadian(gpsLat )),
         -sin(degreeToRadian(gpsLon ))*sin(degreeToRadian(gpsLat )),
          cos(degreeToRadian(gpsLat ))}
        };
